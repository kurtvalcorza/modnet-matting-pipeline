...
|---|---|---|---|---|
| `modnet_matting_colab.ipynb` (`E2E`) | `aa26e44` / `ab5bea58` | 2026-09-20 | Kaggle Tesla T4 (`kurtvalcorza/dimer-nb2-modnet-matting` v2; image `torch 2.10.0+cu128` before the pinned install, `torch 2.14.0+cu130`, `pillow 11.3.0`, `numpy 2.5.3` after, Python 3.12.13, `cuda`) | **PASSED** — 11/11 code cells ok (1 restart after install cell); 6 files, 52 MB fetched into a clean runtime (Hub snapshot only; the portraits are rendered in the kernel); comparison test MAD (all-background 0.341 / all-foreground 0.659): frozen 0.0794 → adapted 0.0035, unknown-band MAD 0.0854 → 0.0235, SAD 20.81 → 0.92 (best epoch 5, validation loss 0.8013 → 0.0584, validation MAD 0.0640 → 0.0040); photograph gate off; reload parity mad_diff: 0.0, metrics_identical: True, max_abs_matte_diff: 0.0; run summary and executed notebook archived under `.agent/backups/kaggle-e2e-2026-09-19/out/dimer-nb2-modnet-matting/v2/evidence/` in the workspace |
...
|---|---|---|---|---|---|
| 2026-09-20 | `aa26e44` / `ab5bea58` | Kaggle Tesla T4 (`kurtvalcorza/dimer-nb2-modnet-matting` v2; image `torch 2.10.0+cu128` before the pinned install, `torch 2.14.0+cu130`, `pillow 11.3.0`, `numpy 2.5.3` after, Python 3.12.13, `cuda`) | Default sample path, `Run all` from a fresh interpreter with an empty Hugging Face cache and no repository checkout (blob SHA-1 verified against GitHub before execution); the checkpoint audited and converted in the notebook, the portraits rendered by the notebook | 234.5 s | **PASSED** — 11/11 code cells ok (1 restart after install cell); 6 files, 52 MB fetched into a clean runtime (Hub snapshot only; the portraits are rendered in the kernel); comparison test MAD (all-background 0.341 / all-foreground 0.659): frozen 0.0794 → adapted 0.0035, unknown-band MAD 0.0854 → 0.0235, SAD 20.81 → 0.92 (best epoch 5, validation loss 0.8013 → 0.0584, validation MAD 0.0640 → 0.0040); photograph gate off; reload parity mad_diff: 0.0, metrics_identical: True, max_abs_matte_diff: 0.0; run summary and executed notebook archived under `.agent/backups/kaggle-e2e-2026-09-19/out/dimer-nb2-modnet-matting/v2/evidence/` in the workspace |
